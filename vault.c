...
void *arrayGenerationThread(void *arg)
{
	...
	if (CIRCULAR_ARRAY == true)
	{
		...
		while (data->circularArray->producerFinished == 0)
		{
			...
			{
				if (DEBUG)
					printf("arrayGenerationThread(), inside for loop %lu...\n", j);
	...
	{
		// logic to hash generation, organize, and store
	}
...
off_t byteArrayToUnsignedIntBigEndian(const uint8_t *byteArray, int b)
{
	...
	printf("byteArrayToUnsignedIntBigEndian(): result1=%ld\n", result);
	...
	printf("byteArrayToUnsignedIntBigEndian(): result2=%ld\n", result);
...
void printFile(const char *filename, int numRecords)
{
	...
	while ((int)recordsPrinted < numRecords && fread(&number, sizeof(MemoRecord), 1, file) == 1)
	{
...
void printFileTail(const char *filename, int numRecords)
{
	...
	{
		printf("printFileTail(): Error seeking in file at offset %lu\n", offset);
		fclose(file);
		exit(EXIT_FAILURE);
	}
...
int binarySearch(const uint8_t *targetHash, size_t targetLength, int fileDescriptor, long long filesize, int *seekCount, bool bulk)
{
	...
	while (left <= right && right - left > SEARCH_SIZE)
	{
		...
		{
			printf("binarySearch(): Error seeking in file at offset %lu\n", middle * sizeof(MemoRecord));
			exit(EXIT_FAILURE);
		}
	...
	if (right - left <= SEARCH_SIZE)
	{
		...
		{
			printf("binarySearch(2): Error seeking in file at offset %lu\n", left * sizeof(MemoRecord));
			exit(EXIT_FAILURE);
		}
...
void *read_bucket(void *arg)
{
	...
	for (int b = 0; b < num_buckets_to_process; ++b)
	{
		if (b == threadID)
		{
			...
			if (bytesRead < 0 || bytesRead != (long long)(BUCKET_SIZE * sizeof(MemoRecord)))
			{
...
{
	ThreadSortArgs *threadArgs = (ThreadSortArgs *)args;

	// Sort the bucket contents
	if (DEBUG)
		printf("sorting bucket before flush...\n");

	if (HASHSORT)
		qsort(threadArgs->records, threadArgs->size, sizeof(MemoRecord), compareMemoRecords);
...
int main(int argc, char *argv[])
{
	...
	int opt;
	while ((opt = getopt(argc, argv, "t:o:m:k:f:q:s:p:r:a:l:c:d:i:x:v:b:y:z:g:w:h")) != -1)
	{
		switch (opt)
		{
		/*case 'w':
		...
		case 'g':
			...
			if (benchmark == false)
				if (DEBUG)
					printf("CIRCULAR_ARRAY=%s\n", CIRCULAR_ARRAY ? "true" : "false");
			break;
		case 'w':
			if (strcmp(optarg, "true") == 0)
			{
	...
	if (FILESIZE == 0 || (long long unsigned)(FILESIZE * 1024 * 1024) > bytes_free)
	...
	else if (targetHash != NULL)
	{
		...
		if (index >= 0)
		{
			...
			size_t bytesRead = read(fd, &foundNumber, sizeof(MemoRecord));
			if (bytesRead != sizeof(MemoRecord))
			{
				perror("Failed to read MemoRecord");
			}
			...
			unsigned long long nonceValue = 0;
			for (size_t i = 0; i < sizeof(foundNumber.nonce); i++)
			{
	...
	else
	{
		...
		if (HASHGEN == true)
		{
			...
			for (int i = 0; i < NUM_THREADS; i++)
			{
				if (CIRCULAR_ARRAY == true)
					...
					threadData[i].circularArray = NULL;
			...
			if (CIRCULAR_ARRAY == true)
			{
				...
				ThreadSortArgs *argsArray[num_threads_sort];
				int threadCount = 0;
						...
						else
						{
							...
							if (bucket->flush < FLUSH_SIZE)
							{
								...
								if (FLUSH_SIZE == 1)
								{
									...
									if (threadCount >= num_threads_sort)
									{
										// Wait for all threads to finish
										for (int i = 0; i < threadCount; i++)
										{
											if (DEBUG)
												printf("waiting for sort thread %d to finish...\n", i);
											pthread_join(threads_sort[i], NULL);
											// Write the bucket to disk after sorting is done
											if (DEBUG)
			...
			{
				// logic without circular array
			}
			// end if statement
		}
		// end of block
		// sort without hashing
		else
		{
			if (benchmark == false)
		...
		if (FLUSH_SIZE > 1 && doSort && HASHSORT)
		{
			...
			for (unsigned long long i = 0; i < (size_t)NUM_BUCKETS; i = i + num_threads_sort)
			{
				if (DEBUG)
